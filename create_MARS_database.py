...
if __name__ == '__main__':
    ...
    if len(ids) < 2:
        raise ValueError('no database was fond')
    for my_id in ids:
        images = sorted(os.listdir(os.path.join(data_dir, my_id)))
        for image in images:
            if is_image_file(image):
                train_imgs.append(os.path.abspath(os.path.join(data_dir, my_id, image)))
    ...
    for my_id in ids:
        ...
        for image in images:
            if is_image_file(image):
                test_imgs.append(os.path.abspath(os.path.join(data_dir, my_id, image)))
